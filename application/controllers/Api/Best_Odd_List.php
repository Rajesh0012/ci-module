 ...
 * @author 
 */
class Best_Odd_List extends REST_Controller {
        ...
        try {
            ...
            if ((!is_null($getDataArr)) && sizeof($getDataArr) > 0 && checkparam($getDataArr, 'array')) {
                ...
                switch ($getDataArr['method']) {
                    case 'odd_list' :
                        $response = $this->odd_list($getDataArr);
                        break;
                    default :
     ...
     * Date: 10/11/2017
     */
    public function odd_list($getDataArr) {
        ...
        try {
            ...
            if (!empty($user_access_token)) {
               ...
               $next = $page + $limit;
                ...
                if(!empty($sport_id))
                          $oddsArr = $this->Sport_model->get_all_odds($sport_id,$limit,$page);
                else
                          $oddsArr = $this->Sport_model->get_all_odds('',$limit,$page);
                
                
                if (!empty($oddsArr['result_arr'])) {

                    if ($oddsArr['total_record'] <= $next) {
                                   $next = -1;
                        }
                    ...
                    $errorMsgArr['VALUE'] = !empty($oddsArr['result_arr'])?$oddsArr['result_arr']:array();
