...
class Admin_model extends CI_Model {
     ...
     * */
     ...
     public function sportEventList($sid = NULL){
         ...
         try {
                 ...
                 $this->db->join($this->prefix . 'sports_events se', 'trnm.tournament_id=se.tournament_id', 'left');
    ...
    public function eventMatches($sid = ''){
        ...
        try {
            ...
            $this->db->select('GROUP_CONCAT(c.competitor_name SEPARATOR "||") as competitor_name,GROUP_CONCAT(DISTINCT(m.id) SEPARATOR ",") as markets,se.tournament_id,se.sportsradar_category,se.event_id');
            $this->db->from($this->prefix . 'sports_events se');
            $this->db->join($this->prefix . 'competitors c','se.event_id=c.event_id','left');
            $this->db->join($this->prefix . 'markets m','se.event_id=m.match_id','left');
            ...
            if (isset($err['code']) && $err['code'] != 0) {
                ...
                foreach ($query->result() as $key=>$values): if(!empty($values->competitor_name))$exp=explode('||',$values->competitor_name);else 'N/A'; ?>

                    <option value="<?php echo  base64_encode($values->sportsradar_category).'&tournament_id='.base64_encode($values->tournament_id).'&matchId='.base64_encode($values->event_id).'&markets='.base64_encode($values->markets).'&filter=2';?>"><?php echo $exp[0].' Vs '.$exp[1]; ?></option>
     ...
     public function countComptitors($id = '',$tournament_id = '')
     {
        try {

            $table = $this->prefix . 'comptitor_list';
			if (!empty($id) && !empty($tournament_id) && !empty($table)) {
            
				
                $this->db->select("count(id) as id");
                $this->db->from($table);
                !empty($id)?$this->db->where('category_id', $id):'';
                !empty($id)?$this->db->where('tournament_id', $tournament_id):'';
                $this->db->group_by('comptitors_id');
                $count = $this->db->get();
                $err = $this->db->error();
                if (isset($err['code']) && $err['code'] != 0) {
                    throw new Exception($this->lang->line('somthing_went_wrong'));
    ...
    public function getComptitorOdds($limit,$offset,$id = '',$tournament_id = '',$matchId = '')
    {

        try {

            if (!empty($id) && !empty($tournament_id)) {
    ...
    public function getMatcheMarketOdds($markets=NULL){
        ...
        try{
            ...
            $this->db->select("mo.type as name,GROUP_CONCAT(mo.book_id SEPARATOR '|+|') as bookies,GROUP_CONCAT(mo.odds SEPARATOR '|+|') as odds,m.market_name,m.category_id");
